...
namespace justine
{
  namespace sampleclient
  {
    ...
    typedef boost::graph_traits <
      NodeRefGraph >::vertex_iterator NRGVertexIter;
    ...
    typedef boost::property_map < NodeRefGraph,
      boost::vertex_name_t >::type VertexNameMap;
    typedef boost::property_map < NodeRefGraph,
      boost::vertex_index_t >::type VertexIndexMap;
    typedef boost::iterator_property_map < NRGVertex *, VertexIndexMap,
      NRGVertex, NRGVertex & >PredecessorMap;
    typedef boost::iterator_property_map < int *, VertexIndexMap, int,
      int &>DistanceMap;
    typedef boost::property_map < NodeRefGraph,
...
*/
      MyShmClient (const char *shm_segment,
		   std::string teamname):ShmClient (shm_segment),
	m_teamname (teamname)
      {
	nr_graph = bgl_graph ();
#ifdef DEBUG
...
*/
	...
	for (justine::robocar::shm_map_Type::iterator iter =
	     shm_map->begin (); iter != shm_map->end (); ++iter)
	  {
...
*/
	...
	{
	0};
	for (vi = boost::vertices (*nr_graph); vi.first != vi.second;
	     ++vi.first)
	  {
	    if (more)
	      if (++count > more)
...
*/
	...
	for (boost::tie (ei, ei_end) = boost::edges (*nr_graph); ei != ei_end;
	     ++ei)
	  {
...
*/
	...
	for (justine::robocar::shm_map_Type::iterator iter =
	     shm_map->begin (); iter != shm_map->end (); ++iter)
	  {
		...
		int to =
		  std::distance (iter->second.m_alist.begin (), noderefi);
		boost::add_edge (f, t, palist (iter->first, to), *nr_graph);
	      }
	  }
#ifdef DEBUG
	std::cout << "# vertices count: " << count << std::endl;;
	std::cout << "# BGF edges: " << boost::num_edges (*nr_graph) << std::
	  endl;;
	std::cout << "# BGF vertices: " << boost::
	  num_vertices (*nr_graph) << std::endl;;
#endif
	return nr_graph;
      }
/**
...
*/
	...
	hasDijkstraPath (osmium::unsigned_object_id_type from,
	...
	VertexIndexMap vertexIndexMap =
	  boost::get (boost::vertex_index, *nr_graph);
	...
	std::cout << std::chrono::duration_cast < std::chrono::milliseconds >
	  (std::chrono::high_resolution_clock::now () -
	   start).count () << " ms " << dist << " meters" << std::endl;
	std::copy (path.begin (), path.end (),
		   std::ostream_iterator < osmium::unsigned_object_id_type >
		   (std::cout, " "));
#endif
	return path;
      }
...
*/
      std::vector < osmium::unsigned_object_id_type >
	hasBellmanFordPath (osmium::unsigned_object_id_type from,
			    osmium::unsigned_object_id_type to)
      {
#ifdef DEBUG
	auto start = std::chrono::high_resolution_clock::now ();
	...
	EdgeWeightMap weightMap =
	  boost::get (boost::edge_weight_t (), *nr_graph);
	PredecessorMap predecessorMap (&parents[0], vertexIndexMap);
	DistanceMap distanceMap (&distances[0], vertexIndexMap);
	boost::bellman_ford_shortest_paths (*nr_graph,
					    boost::num_vertices (*nr_graph),
					    boost::
					    weight_map
					    (weightMap).distance_map
					    (distanceMap).
					    predecessor_map (predecessorMap));
	VertexNameMap vertexNameMap =
	  boost::get (boost::vertex_name, *nr_graph);
...
*/
	...
	int step;
      ...
      typedef int Cop;
      std::vector < Gangster >
	gangsters (boost::asio::ip::tcp::socket & socket, int id,
      ...
      void car (boost::asio::ip::tcp::socket & socket, int id, unsigned *f,
		unsigned *t, unsigned *s);
      void route (boost::asio::ip::tcp::socket & socket, int id,
		  std::vector < osmium::unsigned_object_id_type > &);
    };
  }
